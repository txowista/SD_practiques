...
if [ $PHASE1 = "true" ]; then
# phase 1
	java -cp ../bin recipes_service.test.Phase1TestServer $* &
	sleep 1
#	java -cp ../bin:../../LSim-libraries/* recipes_service.Phase1 $*
	...
	if [ $LOCAL_TEST_SERVER = "true" ]; then
		java -cp ../bin:../../LSim-libraries/* recipes_service.test.TestServer $* &
		sleep 1
	fi

	sleep 1

	java -cp ../bin:../../LSim-libraries/* recipes_service.test.SendArgsToTestServer $*
	...
	#		java -cp ../bin:../../LSim-libraries/* recipes_service.Server $* >$FILE &
			java -cp ../bin:../../LSim-libraries/* recipes_service.Server $* &
		fi
	done
fi
